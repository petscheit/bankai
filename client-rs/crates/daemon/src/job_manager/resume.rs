...
pub async fn update_job_status_for_resume(tx: mpsc::Sender<Job>, db_manager: Arc<DatabaseManager>, job: Job) -> Result<(), DaemonError> {
    ...
    if let Some(job_data) = job_data {
        ...
        let new_status = match job_data.job_status {
            JobStatus::Created | JobStatus::ProgramInputsPrepared | JobStatus::StartedFetchingInputs | JobStatus::StartedTraceGeneration | JobStatus::PieGenerated => JobStatus::Created,
