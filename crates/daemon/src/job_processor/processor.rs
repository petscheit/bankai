...
use bankai_core::types::proofs::epoch_batch::{EpochUpdateBatch, EpochUpdateBatchInputs, ExpectedEpochBatchOutputs};
...
impl JobProcessor {
    pub fn new(db_manager: Arc<DatabaseManager>, bankai: Arc<BankaiClient>) -> Self {
        Self {
            sync_committee_job_processor: Arc::new(SyncCommitteeJobProcessor::new(db_manager.clone(), bankai.clone())),
            epoch_batch_job_processor: Arc::new(EpochBatchJobProcessor::new(db_manager.clone(), bankai.clone())),
            db_manager,
            bankai,
        }
    }
    ...
    pub async fn process_trace_gen_job(&self, job: Job) -> Result<(), DaemonError> {
        match job.job_type {
            JobType::SyncCommitteeUpdate => {
                if let Err(e) = self.sync_committee_job_processor.process_job(job.clone()).await {
            ...
            JobType::EpochBatchUpdate => {
                if let Err(e) = self.epoch_batch_job_processor.process_job(job.clone()).await {
                    self.handle_job_error(job.job_id).await?;
    ...
    pub async fn process_proof_job(&self, job: Job) -> Result<(), DaemonError> {
        match job.job_status {
            JobStatus::OffchainProofRequested => {
                if let Err(e) = process_offchain_proof_stage(job.clone(), self.db_manager.clone(), self.bankai.clone()).await {
                    self.handle_job_error(job.job_id).await?;
            ...
            JobStatus::WrapProofRequested => {
                match job.job_type {
                    JobType::SyncCommitteeUpdate => {
                        if let Err(e) = process_committee_wrapping_stage(job.clone(), self.db_manager.clone(), self.bankai.clone()).await {
                            self.handle_job_error(job.job_id).await?;
                    ...
                    JobType::EpochBatchUpdate => {
                        if let Err(e) = process_epoch_batch_wrapping_stage(job.clone(), self.db_manager.clone(), self.bankai.clone()).await {
                            self.handle_job_error(job.job_id).await?;
    ...
    pub async fn handle_job_error(&self, job_id: Uuid) -> Result<(), DaemonError> {
        ...
        self.db_manager.set_failure_info(job_id, job_data.job_status).await?;
        self.db_manager.update_job_status(job_id, JobStatus::Error).await?;
        Ok(())
    }
