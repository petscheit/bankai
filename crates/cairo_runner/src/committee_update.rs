...
use crate::{
    ...
    types::{Felt, UInt384, Uint256, Uint256Bits32},
};
use cairo_vm::{
...
impl CustomHintProcessor {
    pub fn write_committee_update_inputs(
        ...
        if let Some(committee_update) = &self.committee_input {
            ...
            for (i, branch) in committee_update
                .circuit_inputs
    ...
    pub fn assert_committee_update_result(
        ...
        let expected_outputs = &self
            .committee_input
            .as_ref()
            .expect("Committee input not found")
            .expected_circuit_outputs;

        let state_root_ptr = get_relocatable_from_var_name(
            "state_root",
            vm,
            &hint_data.ids_data,
            &hint_data.ap_tracking,
        ...
        let slot_ptr =
            get_relocatable_from_var_name("slot", vm, &hint_data.ids_data, &hint_data.ap_tracking)?;
        let slot = Felt::from_memory(vm, slot_ptr)?;
        let expected_slot = &expected_outputs.slot;
        if &slot != expected_slot {
            return Err(HintError::AssertionFailed(
                format!("Invalid slot: {:?} != {:?}", slot, expected_slot).into_boxed_str(),
            ));
        }

        let committee_hash_ptr = get_relocatable_from_var_name(
            "committee_hash",
            vm,
            &hint_data.ids_data,
            &hint_data.ap_tracking,
