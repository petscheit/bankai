...
use crate::{
    ...
    types::{Bytes32, Felt, G1CircuitPoint, G2CircuitPoint, Uint256, Uint256Bits32},
};
...
impl CustomHintProcessor {
    ...
    pub fn assert_epoch_update_result(
        ...
        let expected_outputs = &self
            .epoch_input
            .as_ref()
            .expect("EpochUpdate input not found")
...
pub fn assert_epoch_update_result(
    ...
    if slot != expected_outputs.slot {
        return Err(HintError::AssertionFailed(
            format!("Slot Mismatch: {:?} != {:?}", slot, expected_outputs.slot).into_boxed_str(),
        ));
    }
