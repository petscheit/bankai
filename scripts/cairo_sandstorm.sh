...
if [ ! -f "cairo_programs/build/inputs/private_$1" ]; then
    echo "Error: cairo_programs/build/inputs/private_$1 does not exist."
    exit 1
fi
...
cargo +nightly run --manifest-path ./sandstorm/Cargo.toml -p sandstorm-cli -r -F parallel -- \
    ...
    prove --air-private-input cairo_programs/build/inputs/private_$1 \
    --output cairo_programs/proofs/$FILENAME.proof
